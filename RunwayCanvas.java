...
public class RunwayCanvas extends Canvas {
    ...
    public void render() {
        ...
        if (data != null) {
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 4; j++) {
                    g.strokeLine(55 + scaledOffset + 1000 * (data.threshold / maxLength), 355 + i * 25 + j * 5,
                            70 + scaledOffset + 1000 * (data.threshold / maxLength), 355 + i * 25 + j * 5);
                }
            }
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 3; j++) {
                    g.strokeLine(115 + scaledOffset + 1000 * (data.threshold / maxLength), 355 + i * 30 + j * 5,
                            125 + scaledOffset + 1000 * (data.threshold / maxLength), 355 + i * 30 + j * 5);
                }
            }
            g.setLineWidth(1);
            g.save();
            g.translate(88 + scaledOffset + 1000 * (data.threshold / maxLength), 357);
            g.rotate(90);
